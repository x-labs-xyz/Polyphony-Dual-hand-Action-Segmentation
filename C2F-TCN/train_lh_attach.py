...
my_parser.add_argument('--dataset_name', type=str, default="attach", choices=['attach','breakfast', '50salads', 'gtea','assembly_view0_lh_pt'])
...
if args.dataset_name == "breakfast":
    ...
    config.ensem_weights = [1, 1, 1, 1, 0, 0]
elif args.dataset_name == "assembly_view0_lh_pt":
    config.chunk_size = 4
    config.max_frames_per_video = 3800
    config.learning_rate = 5e-4
    config.weight_decay = 3e-4
    config.batch_size = 100
    config.num_class = 75
    config.back_gd = ['null']
    config.ensem_weights = [1, 1, 1, 1, 0, 0]
elif args.dataset_name == "attach":
    config.chunk_size = 4
    config.max_frames_per_video = 15755
    config.learning_rate = 5e-4
    config.weight_decay = 3e-4
    config.batch_size = 100
    config.num_class = 25
...
config.output_dir = config.base_dir + "results/supervised_C2FTCN/left_hand/"
...
config.train_split_file = config.base_dir + "splits/left_hand/train.split{}.bundle".format(config.split_number)
config.test_split_file = config.base_dir + "splits/left_hand/test.split{}.bundle".format(config.split_number)
config.features_file_name = config.base_dir + "/i3d_features/"
...
config.ground_truth_files_dir = config.base_dir + "/groundTruth_downsampled_trimmed/left_hand/"
#config.label_id_csv = config.base_dir + "mapping.csv"
config.label_id_csv = config.base_dir + "mapping.txt"
