...
def evaluate(global_step, net, testloader, run, savedir, logdir):
    print("TESTING" + "~"*10)
    ...
    # Log evaluation results to text file
    eval_log_file = os.path.join(logdir, 'evaluation_results.txt')
    with open(eval_log_file, 'a') as f:
        f.write(f"Step {global_step+1}: {string}\n")
...
if __name__ == '__main__':
    ...
    for eidx in range(start_epoch, cfg.epoch):
        ...
        for batch_idx, (vnames, seq_list, train_label_list, eval_label_list) in enumerate(trainloader):
            ...
            if global_step != 0 and (global_step+1) % cfg.aux.eval_every == 0:
                test_ckpt = evaluate(global_step, net, testloader, run, savedir, logdir)
    ...
    # Log final best results to text file
    eval_log_file = os.path.join(logdir, 'evaluation_results.txt')
    with open(eval_log_file, 'a') as f:
        f.write(f"\n=== FINAL BEST RESULTS (Step {best_ckpt.iteration}) ===\n")
