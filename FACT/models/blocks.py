...
class FACT(nn.Module):
    ...
    def __init__(self, cfg, in_dim, n_classes):
        ...
        if not cfg.FACT.trans : # when video transcript is not available at training and inference, ntoken=60, a_dim=512
    ...
    def _forward_one_video(self, seq, transcript=None):
        ...
        if self.cfg.FACT.cmr: #channel masking dropout
        ...
        if not self.cfg.FACT.trans:
            action_pe = self.action_query # M, B(=1), H (number of tokens, 1, hidden_dim)
    ...
    def forward(self, seq_list, label_list, compute_loss=False):
        ...
        for i, (seq, label) in enumerate(zip(seq_list, label_list)):
            seq = seq.unsqueeze(1)
            trans = torch_class_label_to_segment_label(label)[0] #Example: [2,2,2,5,5,3] → transcript [2,5,3], segment_label [0,0,0,1,1,2].
...
class InputBlock(Block):
    ...
    def forward(self, frame_feature, action_feature, frame_pos, action_pos, action_clogit=None):
        ...
        # action branch, sca decoder(query is frame, key and value are action)
