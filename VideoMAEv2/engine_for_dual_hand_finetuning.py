# Engine for dual-hand video action recognition training
import math
import sys
...
def train_dual_hand_batch(model, samples_dict, targets_dict, criterion):
    """Train batch for dual hand model"""
    # Model expects dict input for dual mode
    outputs = model(samples_dict)
    
    # Compute loss for each hand
    ...
    total_loss = (lh_loss + rh_loss) / 2.0
    
    return total_loss, outputs, {'lh_loss': lh_loss.item(), 'rh_loss': rh_loss.item()}
...
def train_one_epoch_dual(
    ...
    for data_iter_step, batch_data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if isinstance(batch_data, dict):
            ...
            # if batch_data.get('multiple_samples', False) and isinstance(lh_frames, list):
            #     # For repeated augmentation
            #     lh_frames = lh_frames[0]  # Take first augmentation for now
            #     rh_frames = rh_frames[0]
            #     lh_labels = lh_labels[0] if isinstance(lh_labels, list) else lh_labels
            #     rh_labels = rh_labels[0] if isinstance(rh_labels, list) else rh_labels
        ...
        if loss_scaler is None:
            ...
            loss, outputs, loss_dict = train_dual_hand_batch(model, samples_dict, targets_dict, criterion)
        else:
...
@torch.no_grad()
def validation_one_epoch_dual(data_loader, model, device):
    ...
    print('* Overall Loss: {:.3f}'.format(metric_logger.loss.global_avg))

    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}


@torch.no_grad()
def final_test_dual(data_loader, model, device, file_prefix):
    """Test and save predictions for both hands"""
    criterion = torch.nn.CrossEntropyLoss()
