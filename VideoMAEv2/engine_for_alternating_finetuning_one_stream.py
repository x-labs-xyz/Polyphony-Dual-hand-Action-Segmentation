...
def train_one_epoch_alternating(
    ...
    # Create iterators for datasets (RH may be None for left-only mode)
    lh_iterator = iter(metric_logger.log_every(lh_data_loader, print_freq * 2, header + ' [LH]'))
    rh_iterator = None if rh_data_loader is None else iter(metric_logger.log_every(rh_data_loader, print_freq * 2, header + ' [RH]'))
    ...
    for data_iter_step in range(len(lh_data_loader)):
        ...
        # Switch hands every alternation_steps (only if RH is available)
        if rh_iterator is not None and steps_since_switch >= alternation_steps:
        ...
        try:
            if current_hand == 'lh':
                ...
                # If RH is unavailable (left-only), fallback to LH
                if rh_iterator is None:
                    samples, targets, _, _ = next(lh_iterator)
                    current_hand = 'lh'
                else:
...
def validation_one_epoch_alternating(lh_data_loader, rh_data_loader, model, device):
    ...
    # Evaluate right hand (optional)
    if rh_data_loader is not None:
