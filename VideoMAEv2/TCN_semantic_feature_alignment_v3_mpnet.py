...
class ResidualProjector(nn.Module):
    """Enhanced projector with residual connections for better gradient flow"""
    
    def __init__(self, input_dim: int, output_dim: int, hidden_dim: int = None, dropout: float = 0.1):
        super().__init__()
        hidden_dim = hidden_dim or min(input_dim, output_dim)
        
        self.main_path = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.LayerNorm(hidden_dim),
            nn.GELU(),  # Better activation for semantic tasks
            nn.Dropout(dropout),
            nn.Linear(hidden_dim, output_dim),
            nn.LayerNorm(output_dim)
        )
        
        # Skip connection when dimensions differ
...
class AttentionProjector(nn.Module):
    """Self-attention enhanced projector for better feature alignment"""
    ...
    def __init__(self, input_dim: int, output_dim: int, num_heads: int = 8, dropout: float = 0.1):
        ...
        self.projector = ResidualProjector(input_dim, output_dim, dropout=dropout)
...
class TemporalBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int, 
        ...
        self.bn1 = nn.BatchNorm1d(out_channels)  # Add batch normalization
        self.relu1 = nn.ReLU()
        ...
        self.bn2 = nn.BatchNorm1d(out_channels)  # Add batch normalization
        self.relu2 = nn.ReLU()
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        out = self.chomp1(out)
        out = self.bn1(out)  # Apply batch norm after chomping
        out = self.relu1(out)
        out = self.dropout1(out)
        
        out = self.conv2(out)
        out = self.chomp2(out)
        out = self.bn2(out)  # Apply batch norm after chomping
        out = self.relu2(out)
...
class SemanticFeatureAlignmentModel(nn.Module):
    ...
    def __init__(
        ...
        tcn_hidden_dims: List[int] = [640, 512, 384],
        kernel_size: int = 3,
        dropout: float = 0.2,
        ...
        # Enhanced projection with attention and residual connections
        self.visual_projector = AttentionProjector(
            input_dim=tcn_hidden_dims[-1],
            output_dim=semantic_dim,
            num_heads=8,
            dropout=dropout * 0.5  # Lighter dropout for projector
        )
        
        # Enhanced semantic feature processor with residual connections
        self.semantic_projector = ResidualProjector(
            input_dim=semantic_dim,
            output_dim=semantic_dim,  # Keep same dimension
            hidden_dim=alignment_dim,
            dropout=dropout * 0.5
        )
...
class EnhancedMPNetLoss(nn.Module):
    """Enhanced multi-objective loss for MPNet semantic alignment"""
    
    def __init__(self, loss_type: str = 'enhanced_mpnet', alpha: float = 0.6, beta: float = 0.3, 
                 gamma: float = 0.1):
        super().__init__()
        self.loss_type = loss_type
        self.alpha = alpha  # Frame-level alignment
        self.beta = beta    # Sequence-level coherence
        self.gamma = gamma  # Temporal consistency
        
    def forward(self, predicted_features: torch.Tensor, target_features: torch.Tensor, 
                mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        Enhanced loss combining frame, sequence, and temporal objectives
        """
        if self.loss_type == 'enhanced_mpnet':
            # 1. Frame-level cosine similarity
            pred_norm = F.normalize(predicted_features, dim=-1)
            target_norm = F.normalize(target_features, dim=-1)
            frame_cosine = torch.sum(pred_norm * target_norm, dim=-1)  # [B, T]
            frame_loss = 1 - frame_cosine
            
            # 2. Sequence-level global alignment
            if mask is not None:
                # Masked average pooling
                pred_seq = (pred_norm * mask.unsqueeze(-1)).sum(dim=1) / mask.sum(dim=1, keepdim=True)
                target_seq = (target_norm * mask.unsqueeze(-1)).sum(dim=1) / mask.sum(dim=1, keepdim=True)
            else:
                pred_seq = pred_norm.mean(dim=1)  # [B, D]
                target_seq = target_norm.mean(dim=1)  # [B, D]
            
            seq_cosine = F.cosine_similarity(pred_seq, target_seq, dim=-1)
            seq_loss = 1 - seq_cosine
            
            # 3. Temporal consistency (smoothness)
            if predicted_features.size(1) > 1:
                pred_diff = pred_norm[:, 1:] - pred_norm[:, :-1]
                target_diff = target_norm[:, 1:] - target_norm[:, :-1]
                temp_loss = F.mse_loss(pred_diff, target_diff, reduction='none').mean(-1)
                
                # Apply mask to temporal loss
                if mask is not None and mask.size(1) > 1:
                    temp_mask = mask[:, 1:] * mask[:, :-1]  # Both frames must be valid
                    temp_loss = temp_loss * temp_mask
                    temp_loss = temp_loss.sum() / temp_mask.sum() if temp_mask.sum() > 0 else temp_loss.mean()
                else:
                    temp_loss = temp_loss.mean()
            else:
                temp_loss = torch.tensor(0.0, device=predicted_features.device)
            
            # Combine all losses
            if mask is not None:
                frame_loss = (frame_loss * mask).sum() / mask.sum() if mask.sum() > 0 else frame_loss.mean()
            else:
                frame_loss = frame_loss.mean()
            
            return (self.alpha * frame_loss + 
                   self.beta * seq_loss.mean() + 
                   self.gamma * temp_loss)
        
        # Fallback to original adaptive loss for backward compatibility
...
class AdaptiveAlignmentLoss(nn.Module):
    ...
    def __init__(self, loss_type: str = 'adaptive', alpha: float = 0.7, temperature: float = 0.07):
        super().__init__()
        self.loss_type = loss_type
        self.alpha = alpha  # Weight for cosine vs MSE loss
    ...
    def forward(self, predicted_features: torch.Tensor, target_features: torch.Tensor, 
        ...
        if self.loss_type == 'adaptive':
            # Combine cosine and MSE losses for better alignment
            pred_norm = F.normalize(predicted_features, dim=-1)
            target_norm = F.normalize(target_features, dim=-1)
            ...
            # Add contrastive learning for better feature separation
            loss = self._contrastive_loss(predicted_features, target_features, mask)
    ...
    def _contrastive_loss(self, visual_features: torch.Tensor, semantic_features: torch.Tensor, 
                         mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        """Contrastive learning loss for better feature alignment"""
        ...
        similarity = torch.bmm(visual_norm, semantic_norm.transpose(-2, -1)) / self.temperature
        
        # Create labels for positive pairs (diagonal elements)
        labels = torch.arange(seq_len, device=visual_features.device).unsqueeze(0).repeat(batch_size, 1)
        
        # Apply mask to similarity if provided
        if mask is not None:
            # Set masked positions to very negative values
            mask_expanded = mask.unsqueeze(-1)  # [B, T, 1]
            similarity = similarity.masked_fill(~mask_expanded.bool(), -1e9)
        
        # Compute cross-entropy loss
        loss = F.cross_entropy(similarity.view(-1, seq_len), labels.view(-1), reduction='none')
        
        if mask is not None:
            mask_flat = mask.view(-1)
            loss = loss * mask_flat
            return loss.sum() / mask_flat.sum() if mask_flat.sum() > 0 else loss.mean()
        
        return loss.mean()

# Backward compatibility aliases
FrameWiseAlignmentLoss = AdaptiveAlignmentLoss
MPNetLoss = EnhancedMPNetLoss
...
class HAVIDSemanticAlignmentTrainer:
    ...
    def __init__(
        ...
        # Enhanced multi-objective loss function for better alignment
        if loss_type == 'enhanced_mpnet':
            self.criterion = EnhancedMPNetLoss(loss_type=loss_type)
        else:
        ...
        # Enhanced layer-wise learning rates with optimized weight decay
        self.optimizer = torch.optim.AdamW([
            {'params': tcn_params, 'lr': learning_rate * 0.5, 'weight_decay': 1e-5},     # Lighter for TCN
            {'params': projector_params, 'lr': learning_rate * 1.0, 'weight_decay': 1e-4} # Moderate for projectors
        ], weight_decay=1e-5)
        
        # Stable learning rate scheduler with longer cycles
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            self.optimizer, T_0=15, T_mult=1, eta_min=1e-6, verbose=True
        )
    ...
    def train(self, num_epochs: int, save_dir: str = './havid_checkpoints', patience: int = 15):
        ...
        for epoch in range(num_epochs):
            ...
            # Update scheduler - cosine annealing doesn't use val_loss
            self.scheduler.step()
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'train_split': 'splits/View0/lh_pt/train.split1.bundle',
        'test_split': 'splits/View0/lh_pt/test.split1.bundle',
        'save_dir': './havid_checkpoints/v3/MPNET/lh_v0',
        'save_feature_dir': './havid_enhanced_features/v3/MPNET/lh_v0',
        'batch_size': 4,  # Restored to original working size
        'learning_rate': 3e-4,
        'num_epochs': 100,
        'device': 'cuda' if torch.cuda.is_available() else 'cpu',
        'visual_dim': 768,  # VideoMAE feature dimension
        'semantic_dim': 768,  # sentence-transformers/all-MiniLM-L6-v2 dimension
        'tcn_hidden_dims': [640, 512, 384], # [512, 512, 256],
        'max_frames': 3000,
        'downsample_rate': 1,  # Set to 2 or 4 if sequences are too long
        'loss_type': 'enhanced_mpnet',
