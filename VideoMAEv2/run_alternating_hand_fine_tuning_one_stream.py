...
from engine_for_alternating_finetuning_one_stream import (
...
def get_args():
    ...
    parser.add_argument('--lh_data_path', required=True, type=str, help='Left hand data path')
    parser.add_argument('--lh_data_root', required=True, type=str, help='Left hand data root')
    parser.add_argument('--rh_data_path', required=False, default='', type=str, help='Right hand data path')
    parser.add_argument('--rh_data_root', required=False, default='', type=str, help='Right hand data root')
    parser.add_argument('--lh_num_classes', default=75, type=int)
    parser.add_argument('--rh_num_classes', default=75, type=int)
    parser.add_argument('--left_only', action='store_true', default=False,
...
def main(args):
    ...
    # If only left-hand data is available, mirror it to the right-hand side
    if args.left_only:
    ...
    # Right hand datasets (mirror LH when --left_only)
    ...
    # Create samplers (RH optional for left-only mode)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if lh_data_loader_val is not None:
            # For left-only mode, skip RH validation (pass None) and ignore RH metrics
            test_stats = validation_one_epoch_alternating(
                lh_data_loader_val, rh_data_loader_val if not args.left_only else None, model, device)
            ...
            if test_stats.get('lh_acc1', 0) > max_lh_accuracy:
                ...
                save_best = True
            if not args.left_only and test_stats.get('rh_acc1', 0) > max_rh_accuracy:
    ...
    file_prefix = os.path.join(args.output_dir, str(global_rank))
    test_stats = final_test_alternating(
        lh_data_loader_test, rh_data_loader_test if not args.left_only else None, model, device, file_prefix)
