...
class TemporalConvNet(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dims: List[int], kernel_size: int = 3, dropout: float = 0.3):
        super().__init__()
...
class SemanticFeatureAlignmentModel(nn.Module):
    ...
    def __init__(
        ...
        # Enhanced projection layers for better semantic alignment
        # Progressive dimension mapping: TCN -> intermediate -> semantic
        intermediate_dim = min(semantic_dim, tcn_hidden_dims[-1] * 2)  # Adaptive intermediate size
        
        self.visual_projector = nn.Sequential(
            # First projection to intermediate space
            nn.Linear(tcn_hidden_dims[-1], intermediate_dim),
            nn.LayerNorm(intermediate_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
            
            # Second projection to match semantic dimension exactly
            nn.Linear(intermediate_dim, semantic_dim),
            nn.LayerNorm(semantic_dim),
            nn.Dropout(dropout * 0.5)  # Lighter dropout at output
        )
...
class HAVIDSemanticAlignmentTrainer:
    ...
    def __init__(
        ...
        # Enhanced loss function with adaptive alignment
        ...
        self.optimizer = torch.optim.AdamW([
            {'params': tcn_params, 'lr': learning_rate * 0.5, 'weight_decay': 1e-4},  # Lower LR for backbone
            {'params': projector_params, 'lr': learning_rate * 1.0, 'weight_decay': 1e-3}  # Standard LR for projectors
        ], weight_decay=1e-4)
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'save_dir': './havid_checkpoints/v2/BAAI/lh_v0',
        'save_feature_dir': './havid_enhanced_features/v2/BAAI/lh_v0',
        'batch_size': 2,  # Increased batch size for better gradients
        'learning_rate': 5e-4,
        'num_epochs': 100,
        'device': 'cuda' if torch.cuda.is_available() else 'cpu',
        'visual_dim': 768,  # VideoMAE feature dimension
        'semantic_dim': 1024,  # sentence-transformers/all-MiniLM-L6-v2 dimension
        'tcn_hidden_dims': [512, 512, 256],
        'max_frames': 3000,
        'downsample_rate': 1,  # Set to 2 or 4 if sequences are too long
        'loss_type': 'adaptive',
        'semantic_embeddings_path': '/home/hao/Polyphony/data/havid/semantic_embeddings/BAAI_bge-large-en-v1.5.pt'
    }
