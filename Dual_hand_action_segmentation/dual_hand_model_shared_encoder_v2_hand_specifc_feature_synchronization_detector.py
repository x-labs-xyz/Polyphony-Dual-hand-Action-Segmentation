...
class SynchronizationHead(nn.Module):
    ...
    def forward(self, feats_lh, feats_rh):
        ...
        sync_score = self.sync_predictor(combined)
        return sync_score
...
class DualHandASDiffusionModel(nn.Module):
    ...
    def __init__(self, encoder_params, decoder_params, diffusion_params, num_classes, device):
        ...
        self.sync_head = SynchronizationHead(decoder_input_dim)

    def compute_sync_loss(self, sync_score, label_lh, label_rh):
        """Predict if hands are doing the same action (synchronized)"""
        # Compute if same action
        label_lh_idx = torch.argmax(label_lh, dim=1, keepdim=True)
        label_rh_idx = torch.argmax(label_rh, dim=1, keepdim=True)
        same_action = (label_lh_idx == label_rh_idx).float()
    
        # BCE loss
        return F.binary_cross_entropy(sync_score, same_action)
    ...
    def get_training_loss(self, video_feats_lh, video_feats_rh,
        ...
        sync_score = self.sync_head(backbone_feats_lh, backbone_feats_rh)
        sync_loss = self.compute_sync_loss(sync_score, event_gt_left, event_gt_right)
