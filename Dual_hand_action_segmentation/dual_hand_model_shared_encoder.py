...
class DualHandASDiffusionModel(nn.Module):
    ...
    def model_predictions(self, backbone_feats, x_left, x_right, t):
        """Make predictions for both hands simultaneously"""
        # Process left hand
        ...
        x_left_start = self.decoder_left(backbone_feats, t, x_left_m.float())
        ...
        x_right_start = self.decoder_right(backbone_feats, t, x_right_m.float())
    ...
    def get_training_loss(self, video_feats, 
        ...
        # Encoder losses (shared for both hands - we could use combined GT or focus on one hand)
        # For simplicity, let's use left hand GT for encoder loss
        encoder_loss_dict = self._compute_encoder_losses(
            encoder_out, event_gt_left, encoder_ce_criterion, 
        ...
        left_losses = self._compute_decoder_losses(
            backbone_feats, event_gt_left, boundary_gt_left, 'left',
            decoder_ce_criterion, decoder_mse_criterion, decoder_boundary_criterion, soft_label
        )
        ...
        right_losses = self._compute_decoder_losses(
            backbone_feats, event_gt_right, boundary_gt_right, 'right',
            decoder_ce_criterion, decoder_mse_criterion, decoder_boundary_criterion, soft_label
        )
        ...
        # Combine all losses
        loss_dict = {**encoder_loss_dict, **decoder_loss_dict}
    ...
    def ddim_sample(self, video_feats, seed=None):
        ...
        for time, time_next in time_pairs:
            ...
            pred_noise_left, x_left_start, pred_noise_right, x_right_start = self.model_predictions(
                backbone_feats, x_left, x_right, time_cond
            )
