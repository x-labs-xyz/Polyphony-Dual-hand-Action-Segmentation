...
from dual_hand_dataset_shared_encoder_hand_specific_feature import get_boundary_seq
...
class DualHandTrainer:
    ...
    def __init__(self, encoder_params, decoder_params, diffusion_params, 
                 ...
                 synthetic_dual_stream_cfg=None, boundary_smooth=None):
        ...
        self.synthetic_dual_stream_cfg = synthetic_dual_stream_cfg or { 'enabled': False }
    ...
    def _perturb_labels_by_shifting_boundaries(self, label_1d_np, max_shift_frames, max_perturbations, min_segment_len=3):
        ...
        if len(candidate_indices) == 0:
            return label_1d_np
        chosen = np.random.choice(candidate_indices, size=min(num_to_perturb, len(candidate_indices)), replace=False)
        ...
        for bi in chosen:
            ...
            new_end_left = max(new_starts[bi] + min_segment_len, min(new_ends[bi] + shift, new_ends[bi+1] - min_segment_len))
        ...
        # Fill any potential gaps due to rounding (shouldn't happen, but guard)
        if new_starts[0] > 0:
    ...
    def train(self, train_train_dataset, train_test_dataset, test_test_dataset, 
        ...
        for epoch in range(restore_epoch+1, num_epochs):
            ...
            for _, data in enumerate(train_train_loader):
                ...
                # Debug: print original boundary shape
                print(f"Original boundary_lh shape: {boundary_lh.shape}, boundary_rh shape: {boundary_rh.shape}")
                
                # Optionally synthesize a perturbed right-hand label stream (Breakfast is single stream)
                if self.synthetic_dual_stream_cfg.get('enabled', False):
                    max_shift = int(self.synthetic_dual_stream_cfg.get('max_shift_frames', 5))
                    max_perturb = int(self.synthetic_dual_stream_cfg.get('max_perturbations', 3))
                    min_seg = int(self.synthetic_dual_stream_cfg.get('min_segment_len', 3))
                    ...
                    label_rh_np = self._perturb_labels_by_shifting_boundaries(label_rh_np, max_shift, max_perturb, min_seg)
                    # Restore batch dimension [1, T]
                    label_rh = torch.from_numpy(label_rh_np).to(device).unsqueeze(0)
                    # Recompute boundary for RH (1D input) and normalize like dataset does, then add batch dim
...
if __name__ == '__main__':
    ...
    feature_dir_lh = os.path.join(root_data_dir, dataset_name, 'features')
    feature_dir_rh = os.path.join(root_data_dir, dataset_name, 'features')
    ...
    # Synthetic dual-stream config (optional)
    if 'synthetic_dual_stream' not in locals():
        synthetic_dual_stream = {
            'enabled': False,
            'max_shift_frames': 5,
            'max_perturbations': 3,
            'min_segment_len': 3
    ...
    trainer = DualHandTrainer(
        ...
        synthetic_dual_stream_cfg=synthetic_dual_stream,
        boundary_smooth=boundary_smooth
    )    
