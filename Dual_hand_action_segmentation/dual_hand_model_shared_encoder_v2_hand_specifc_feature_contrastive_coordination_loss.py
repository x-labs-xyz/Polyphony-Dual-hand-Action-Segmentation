...
class DualHandASDiffusionModel(nn.Module):
    ...
    def compute_contrastive_coordination_loss(self, feats_lh, feats_rh, label_lh, label_rh, temp=0.07):
        """Pull features together when coordinated, push apart when independent"""
        # Normalize features
        feats_lh_norm = F.normalize(feats_lh, dim=1)
        feats_rh_norm = F.normalize(feats_rh, dim=1)
        
        # Compute similarity
        similarity = (feats_lh_norm * feats_rh_norm).sum(dim=1)
    
        # Target: high similarity when same action
        label_lh_idx = torch.argmax(label_lh, dim=1)
        label_rh_idx = torch.argmax(label_rh, dim=1)
        target_sim = (label_lh_idx == label_rh_idx).float()
    
        # MSE loss
        loss = F.mse_loss(torch.sigmoid(similarity / temp), target_sim)
        return loss
    ...
    def get_training_loss(self, video_feats_lh, video_feats_rh,
        ...
        contrastive_loss = self.compute_contrastive_coordination_loss(
            backbone_feats_lh, backbone_feats_rh, event_gt_left, event_gt_right
            )
