...
class DualHandASDiffusionModel(nn.Module):
    ...
    def forward(self, backbone_feats, t, event_diffused_left, event_diffused_right, 
        ...
        if hand in ['left', 'both']:
            backbone_feats_cond = self._apply_conditioning(
                backbone_feats, cond_type, event_gt_left, boundary_gt_left, event_gt_right, boundary_gt_right
            )
            results['left'] = self.decoder_left(backbone_feats_cond, t, event_diffused_left.float())
        
        if hand in ['right', 'both']:
            backbone_feats_cond = self._apply_conditioning(
                backbone_feats, cond_type, event_gt_right, boundary_gt_right, event_gt_left, boundary_gt_left 
            )
            results['right'] = self.decoder_right(backbone_feats_cond, t, event_diffused_right.float())

        return results

    def _apply_conditioning(self, backbone_feats, cond_type, event_gt, boundary_gt, event_gt_other_hand=None, boundary_gt_other_hand=None):
        ...
        elif cond_type == 'cross-hand-action':
            # Mask based on whether the other hand is performing the same action
            if event_gt_other_hand is not None:
                event_gt_idx = torch.argmax(event_gt, dim=1, keepdim=True)
                event_gt_other_idx = torch.argmax(event_gt_other_hand, dim=1, keepdim=True)
            
                # Mask when both hands do the same action (coordinated)
                feature_mask = (event_gt_idx != event_gt_other_idx).float()
                return feature_mask * backbone_feats
            return backbone_feats
    
        elif cond_type == 'cross-hand-boundary-sync':
            # Mask based on boundary synchronization between hands
            if boundary_gt_other_hand is not None:
                # When both hands have boundaries (synchronized transitions)
                both_boundaries = (boundary_gt > 0.3) & (boundary_gt_other_hand > 0.3)
            
                # Mask synchronized boundary regions
                feature_mask = (~both_boundaries).float()
                return feature_mask * backbone_feats
            return backbone_feats
    
        elif cond_type == 'cross-hand-asymmetric':
            # Mask based on action asymmetry (different actions at same time)
            if event_gt_other_hand is not None:
                # Calculate action diversity between hands
                event_gt_idx = torch.argmax(event_gt, dim=1, keepdim=True)
                event_gt_other_idx = torch.argmax(event_gt_other_hand, dim=1, keepdim=True)
            
                # Mask when actions are the same (push model to learn differentiation)
                feature_mask = (event_gt_idx == event_gt_other_idx).float()
                return feature_mask * backbone_feats
            return backbone_feats
    
        elif cond_type == 'alternating-hand':
            # Randomly mask one entire hand to force cross-hand dependency learning
            if event_gt_other_hand is not None:
                # 50% chance to keep features, 50% to zero them out
                if torch.rand(1).item() > 0.5:
                    return torch.zeros_like(backbone_feats)
            return backbone_feats
    
        elif cond_type == 'cross-hand-temporal-offset':
            # Mask based on temporal offset between hand actions
            if event_gt_other_hand is not None and event_gt.shape[2] > 2:
                # Check if other hand's future or past action matches current action
                event_gt_idx = torch.argmax(event_gt, dim=1)  # [B, T]
                event_gt_other_idx = torch.argmax(event_gt_other_hand, dim=1)  # [B, T]
            
                # Check temporal alignment with offset
                offset = 2  # frames
                future_match = torch.zeros_like(event_gt_idx, dtype=torch.bool)
                if event_gt_idx.shape[1] > offset:
                    future_match[:, :-offset] = (event_gt_idx[:, :-offset] == 
                                                 event_gt_other_idx[:, offset:])
            
                # Mask when there's temporal coordination
                feature_mask = (~future_match).float().unsqueeze(1)
                return feature_mask * backbone_feats
            return backbone_feats
    ...
    def get_training_loss(self, video_feats_lh, video_feats_rh,
        ...
        left_losses = self._compute_decoder_losses(
            backbone_feats_lh, event_gt_left, boundary_gt_left, event_gt_right, boundary_gt_right, 'left',
        ...
        right_losses = self._compute_decoder_losses(
            backbone_feats_rh, event_gt_right, boundary_gt_right, event_gt_left, boundary_gt_left, 'right',
    ...
    def _compute_decoder_losses(self, backbone_feats, event_gt, boundary_gt, event_gt_other, boundary_gt_other, hand, 
        ...
        if hand == 'left':
            ...
            event_out = self.forward(backbone_feats, t, event_diffused, event_diffused, 
                                   event_gt, event_gt_other, boundary_gt, boundary_gt_other, hand='left')['left']
        else:  # right
            _, _, event_diffused, noise, t = self.prepare_targets(event_gt, event_gt)  # Use same GT twice for single hand
            event_out = self.forward(backbone_feats, t, event_diffused, event_diffused,
                                   event_gt_other, event_gt, boundary_gt_other, boundary_gt, hand='right')['right']
