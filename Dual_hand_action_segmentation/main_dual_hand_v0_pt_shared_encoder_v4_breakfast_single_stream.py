...
class DualHandTrainer:
    ...
    def __init__(self, encoder_params, decoder_params, diffusion_params, 
                 event_list, sample_rate, temporal_aug, set_sampling_seed, postprocess, device, single_stream=False):
    ...
    def train(self, train_train_dataset, train_test_dataset, test_test_dataset, 
        ...
        for epoch in range(restore_epoch+1, num_epochs):
            ...
            for _, data in enumerate(train_train_loader):
                ...
                # If single-stream, mirror LH to RH to avoid redundant IO/compute
                if self.single_stream:
                    feature_rh = feature_lh
                    label_rh = label_lh
                    boundary_rh = boundary_lh
                ...
                if self.single_stream:
                    # Disable right-hand specific losses when training single-stream
                    current_weights = current_weights.copy()
                    if 'encoder_rh_ce_loss' in current_weights:
                        current_weights['encoder_rh_ce_loss'] = 0.0
                    if 'encoder_rh_mse_loss' in current_weights:
                        current_weights['encoder_rh_mse_loss'] = 0.0
                    if 'encoder_rh_boundary_loss' in current_weights:
                        current_weights['encoder_rh_boundary_loss'] = 0.0
                    if 'decoder_right_ce_loss' in current_weights:
                        current_weights['decoder_right_ce_loss'] = 0.0
                    if 'decoder_right_mse_loss' in current_weights:
                        current_weights['decoder_right_mse_loss'] = 0.0
                    if 'decoder_right_boundary_loss' in current_weights:
                        current_weights['decoder_right_boundary_loss'] = 0.0
            ...
            if epoch % log_freq == 0:
                ...
                for mode in ['decoder-agg']:  # Can add other modes if needed
                    ...
                    if result_dir and not self.single_stream:
                        base_weight = loss_weights.get('decoder_right_ce_loss', 0.0)
                        if base_weight > 0:
                            logger.add_scalar('Adaptive/RightHandBoost', 
                                            adaptive_weights['decoder_right_ce_loss'] / base_weight, 
                                            epoch)
                    ...
                    if not self.single_stream:
                        base_weight = loss_weights.get('decoder_right_ce_loss', 0.0)
                        if base_weight > 0:
                            boost_factor = adaptive_weights['decoder_right_ce_loss'] / base_weight
    ...
    def test_single_video(self, video_idx, test_dataset, mode, device, model_path=None):  
        ...
        with torch.no_grad():
            ...
            if self.single_stream:
                output_rh_processed = output_lh_processed
            else:
    ...
    def test(self, test_dataset, mode, device, label_dir_lh, label_dir_rh, result_dir=None, model_path=None):
        ...
        if not self.single_stream:
            ...
            acc_rh, edit_rh, f1s_rh = acc_lh, edit_lh, f1s_lh
...
if __name__ == '__main__':
    ...
    parser.add_argument('--single_stream', action='store_true', help='Enable single-stream training/evaluation')
    ...
    feature_dir_lh = os.path.join(root_data_dir, dataset_name, 'features_one_stream/concatenated/lh_v0')
    feature_dir_rh = os.path.join(root_data_dir, dataset_name, 'features_one_stream/concatenated/lh_v0')
    label_dir_lh = os.path.join(root_data_dir, dataset_name, 'groundTruth')
    label_dir_rh = os.path.join(root_data_dir, dataset_name, 'groundTruth')
    ...
    dual_hand_loss_weights = {
        ...
        'encoder_rh_ce_loss': 0.0 if args.single_stream else loss_weights.get('encoder_ce_loss', 0.5) / 2,
        'encoder_rh_mse_loss': 0.0 if args.single_stream else loss_weights.get('encoder_mse_loss', 0.025) / 2,
        'encoder_rh_boundary_loss': 0.0 if args.single_stream else loss_weights.get('encoder_boundary_loss', 0.0) / 2,
        ...
        'decoder_right_ce_loss': 0.0 if args.single_stream else loss_weights.get('decoder_ce_loss', 0.5) / 2,
        'decoder_right_mse_loss': 0.0 if args.single_stream else loss_weights.get('decoder_mse_loss', 0.025) / 2,
        'decoder_right_boundary_loss': 0.0 if args.single_stream else loss_weights.get('decoder_boundary_loss', 0.1) / 2,
    }
