...
class CrossHandAttentionLayer(nn.Module):
    def __init__(self, dim, num_heads=4):
        super().__init__()
        self.lh_to_rh = nn.MultiheadAttention(dim, num_heads, batch_first=True)
        self.rh_to_lh = nn.MultiheadAttention(dim, num_heads, batch_first=True)
        self.norm_lh = nn.LayerNorm(dim)
        self.norm_rh = nn.LayerNorm(dim)
        
    def forward(self, feats_lh, feats_rh):
        # B, C, T -> B, T, C for attention
        lh = feats_lh.transpose(1, 2)
        rh = feats_rh.transpose(1, 2)
        
        lh_attended, _ = self.lh_to_rh(lh, rh, rh)
        rh_attended, _ = self.rh_to_lh(rh, lh, lh)
        
        lh = self.norm_lh(lh + lh_attended).transpose(1, 2)
        rh = self.norm_rh(rh + rh_attended).transpose(1, 2)
...
class DualHandASDiffusionModel(nn.Module):
    ...
    def __init__(self, encoder_params, decoder_params, diffusion_params, num_classes, device):
        ...
        self.cross_hand_attention = CrossHandAttentionLayer(
            dim=decoder_input_dim,  # Use same dim as decoder input
            num_heads=4
            )
    ...
    def get_training_loss(self, video_feats_lh, video_feats_rh,
        ...
        backbone_feats_lh, backbone_feats_rh = self.cross_hand_attention(
